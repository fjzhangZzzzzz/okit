...
def test_detect_from_registry_success(mock_winreg: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
...
@patch("okit.tools.mobaxterm_pro.winreg")
def test_detect_from_registry_failure(mock_winreg: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
...
def test_detect_from_paths_success(mock_exists: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
    """Test successful path detection."""
    mock_exists.return_value = True
    
    with patch("okit.tools.mobaxterm_pro.MobaXtermDetector._get_file_version") as mock_get_version:
        ...
        assert result["detection_method"] == "paths"


@patch("okit.tools.mobaxterm_pro.os.path.exists")
def test_detect_from_paths_failure(mock_exists: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
...
def test_detect_from_environment_success(mock_environ: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
...
@patch("okit.tools.mobaxterm_pro.os.environ")
def test_detect_from_environment_failure(mock_environ: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
...
def test_get_file_version_success(mock_run: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
    """Test successful file version retrieval."""
    mock_run.return_value.returncode = 0
    mock_run.return_value.stdout = b"FileVersion=22.0.0.0"
...
@patch("subprocess.run")
def test_get_file_version_failure(mock_run: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
...
@patch("subprocess.run")
def test_get_version_from_command_success(mock_run: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
    """Test successful version retrieval from command."""
    mock_run.return_value.returncode = 0
    mock_run.return_value.stdout = b"22.0"
...
@patch("subprocess.run")
def test_get_version_from_command_failure(mock_run: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
...
def test_resolve_scoop_executable(mobaxterm_detector: MobaXtermDetector) -> None:
    ...
    with patch("okit.tools.mobaxterm_pro.os.path.exists") as mock_exists:
        with patch("okit.tools.mobaxterm_pro.os.path.realpath") as mock_realpath:
            mock_exists.return_value = True
            mock_realpath.return_value = "C:\\Users\\test\\scoop\\apps\\mobaxterm\\22.0\\MobaXterm.exe"
            
            result = mobaxterm_detector._resolve_scoop_executable(shim_path)
            assert result is not None


def test_resolve_chocolatey_executable(mobaxterm_detector: MobaXtermDetector) -> None:
    ...
    with patch("okit.tools.mobaxterm_pro.os.path.exists") as mock_exists:
        with patch("okit.tools.mobaxterm_pro.os.path.realpath") as mock_realpath:
            mock_exists.return_value = True
            mock_realpath.return_value = "C:\\ProgramData\\chocolatey\\lib\\mobaxterm\\tools\\MobaXterm.exe"
...
@patch("subprocess.run")
def test_get_version_from_powershell_success(mock_run: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
    """Test successful version retrieval from PowerShell."""
    mock_run.return_value.returncode = 0
    mock_run.return_value.stdout = b"22.0.0.0"
...
@patch("subprocess.run")
def test_get_version_from_powershell_failure(mock_run: MagicMock, mobaxterm_detector: MobaXtermDetector) -> None:
...
def test_extract_version_from_path(mobaxterm_detector: MobaXtermDetector) -> None:
    ...
    result = mobaxterm_detector._extract_version_from_path("C:\\Program Files\\Mobatek\\MobaXterm 22.0\\MobaXterm.exe")
    assert result == "22.0"
    
    # Test without version in path
    result = mobaxterm_detector._extract_version_from_path("C:\\Program Files\\Mobatek\\MobaXterm\\MobaXterm.exe")
    assert result is None


def test_get_license_file_path(mobaxterm_detector: MobaXtermDetector) -> None:
...
def test_mobaxterm_keygen_initialization(mobaxterm_keygen: MobaXtermKeygen) -> None:
    ...
    assert len(mobaxterm_keygen.variant_base64_table) > 0
...
def test_generate_license_key_invalid_version(mobaxterm_keygen: MobaXtermKeygen) -> None:
...
def test_create_license_file(mobaxterm_keygen: MobaXtermKeygen, temp_license_file: Path) -> None:
    """Test license file creation."""
    username = "testuser"
    version = "22.0"
    
    result = mobaxterm_keygen.create_license_file(username, version, str(temp_license_file))
...
def test_validate_license_file(mobaxterm_keygen: MobaXtermKeygen, temp_license_file: Path) -> None:
...
def test_mobaxterm_pro_tool_initialization(mobaxterm_pro_tool: MobaXtermProTool) -> None:
...
def test_mobaxterm_pro_tool_cli_help(mobaxterm_pro_tool: MobaXtermProTool) -> None:
    ...
    assert "MobaXterm license key generator tool" in help_text
    
    short_help = mobaxterm_pro_tool._get_cli_short_help()
    assert "MobaXterm license key generator tool" in short_help
...
def test_generate_command(mock_keygen_class: MagicMock, temp_license_file: Path) -> None:
    ...
    result = runner.invoke(
        mobaxterm_pro.cli,
        ["generate", "--username", "testuser", "--version", "22.0", "--output-path", str(temp_license_file)]
    )
    assert result.exit_code == 0
...
@patch("okit.tools.mobaxterm_pro.MobaXtermDetector")
@patch("okit.tools.mobaxterm_pro.MobaXtermKeygen")
def test_deploy_command(mock_keygen_class: MagicMock, mock_detector_class: MagicMock) -> None:
    """Test deploy command."""
    mock_detector = MagicMock()
    mock_detector_class.return_value = mock_detector
    mock_detector.detect_installation.return_value = {
...
def test_validate_command(mock_keygen_class: MagicMock) -> None:
    ...
    result = runner.invoke(
        mobaxterm_pro.cli,
        ["validate", "--username", "testuser", "--license-key", "test_key", "--version", "22.0"]
    )
    assert result.exit_code == 0
...
def test_normalize_version(mobaxterm_keygen: MobaXtermKeygen) -> None:
    ...
    with pytest.raises(KeygenError):
        mobaxterm_keygen._normalize_version("invalid")
...
def test_generate_license_with_different_types(mobaxterm_keygen: MobaXtermKeygen) -> None:
    ...
    for license_type in [1, 2, 3]:  # Different license types
        license_key = mobaxterm_keygen._generate_license(license_type, 1, username, 22, 0)
        assert isinstance(license_key, str)
        assert len(license_key) > 0
