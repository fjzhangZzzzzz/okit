...
func TestInstallersVerifyReleaseAndWriteOfficialMetadata(t *testing.T) {
	for _, file := range []string{"install.sh", "install.ps1"} {
		...
		for _, required := range []string{"checksums.txt", "SHA256", "install.json", "official", "OKIT_VERSION"} {
...
func TestGoReleaserBuildsDocumentedMatrixAndPublishesInstallers(t *testing.T) {
	...
	for _, required := range []string{"linux", "windows", "amd64", "arm64", "checksums.txt", "install.sh", "install.ps1", "README.md", "LICENSE"} {
