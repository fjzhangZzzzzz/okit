    ...
    def generate_report(self, metrics: PerformanceMetrics) -> str:
        ...
        if metrics.tools:
            ...
            for i, (tool_name, tool_data) in enumerate(sorted_tools[:6], 1):
                ...
                if total_time > 0:
                    ...
                    lines.append(f"   {i}. {tool_name:<18} {time_str:>8} ({percentage:>5}) {status}")
            lines.append("")
        ...
        if metrics.bottlenecks:
            ...
            for bottleneck in metrics.bottlenecks[:3]:
                ...
                lines.append(f"   • {module} is slow ({time_str}) - {bottleneck.get('type', 'unknown')}")
            
            # Show summary stats
            slow_count = len([b for b in metrics.bottlenecks if b.get("severity") == "high"])
            medium_count = len([b for b in metrics.bottlenecks if b.get("severity") == "medium"])
        ...
        if metrics.total_time > target_time:
            ...
            lines.append(self._colorize(f"🎯 Target: Reduce to <{target_str} (current: {current_str}, need: -{improvement_str})", "yellow"))
        else:
            lines.append(self._colorize(f"🎯 Target achieved: <{target_str} ✓", "green"))
...
class JSONReporter:
    ...
    def generate_report(self, metrics: PerformanceMetrics) -> str:
        ...
        data["status"] = "excellent" if metrics.total_time <= 0.3 else \
                        "good" if metrics.total_time <= 0.6 else \
                        "needs_improvement"
    ...
    def generate_report(
        ...
        if output_file and json_report:
            try:
                ...
                print(f"Warning: Could not save JSON report to {output_file}: {e}", file=sys.stderr)
