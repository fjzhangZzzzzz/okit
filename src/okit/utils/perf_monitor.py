...
class DecoratorTracker:
    ...
    def _timed_okit_tool(self, *args, **kwargs):
...
class RegistrationTracker:
    ...
    def __init__(self):
        ...
        self.original_auto_register = None
        self.original_add_command = None
        self.tracking_enabled = False
    ...
    def _timed_auto_register(self, package, package_path, parent_group, debug_enabled=False):
        """Timed version of auto_register_commands"""
        start_time = time.perf_counter()
        
        # Execute original registration
        result = self.original_auto_register(package, package_path, parent_group, debug_enabled)
...
class PerformanceAnalyzer:
    ...
    def analyze_metrics(self, metrics: PerformanceMetrics) -> PerformanceMetrics:
        ...
        if any(b["module"].endswith("mobaxterm_pro") for b in bottlenecks):
            recommendations.append("Consider lazy loading of cryptography in mobaxterm_pro")
            
        if any(b["module"].endswith("shellconfig") for b in bottlenecks):
        ...
        bottlenecks.sort(key=lambda x: (x["severity"] == "high", x["time"]), reverse=True)
...
def is_monitoring_enabled() -> bool:
    ...
    return (
        os.getenv("OKIT_PERF_MONITOR") is not None or
        os.getenv("OKIT_PERF") is not None or
        "--perf-monitor" in sys.argv
    )
...
def get_perf_config(cli_format: Optional[str] = None, cli_output: Optional[str] = None) -> PerfConfig:
    ...
    env_enabled = bool(os.getenv("OKIT_PERF_MONITOR") or 
                      os.getenv("OKIT_PERF") or 
                      os.getenv("OKIT_PERFORMANCE"))
    ...
    format_type = (cli_format or 
                  os.getenv("OKIT_PERF_MONITOR", "basic"))
    
    # Determine output file: CLI > ENV > None
    output_file = (cli_output or 
                  os.getenv("OKIT_PERF_OUTPUT"))
...
def update_cli_performance_config(cli_format: Optional[str] = None, cli_output: Optional[str] = None) -> None:
